      ...
      <div className="left-side">
        <form onSubmit={handleSubmit(onSubmit)} ><h1>LogIn</h1>{/* input email */}
          <div className="form-control" style={{backgroundColor:'transparent', border :'none' ,padding:'0'} }><label htmlFor="Email"> Email</label>
            ...
            {errors.email && errors.email.type === "required" && (  <span style={{ color: "red" ,marginTop:"10px"}}>This is required</span>)}{errors.email && errors.email.type === "pattern" && (  <span style={{ color: "red" }}>    Entered value does not match email format  </span>)}
          ...
          {...register("password", { required: {  value: true,},minLength: {  value: 8},  maxLength:{  value: 30},pattern: { value:/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/, },})}/>
            {errors.password && errors.password.type === "required" && (<span style={{ color: "red" , marginTop:"10px" }}>This is required</span> )} {errors.password && errors.password.type === "maxLength" && (   <span style={{ color: "red" , marginTop:"10px" }}>Max length exceeded</span> )} {errors.password && errors.password.type === "minLength" && (   <span style={{ color: "red" , marginTop:"10px" }}>Min length 8 Character</span> )} {errors.password && errors.password.type === "pattern" && (   <span style={{ color: "red" , marginTop:"10px" }}>     Entered value does not match email format   </span>)}
          </div>
          {/* button */}
