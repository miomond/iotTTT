...
function Bloger() {
    const props = useSpring({from: { size: '20%' },to: { size: '100%' },config: { duration: 3000 },  })
  return (
    <>
            <Navpar />

   <section className="master" style={props}> <h1>Blog Smart Home</h1> <p>   our creative solution is always there for people to to higher their quality of life    so we always hope to be able to support more devices & categories   here you can find our latest and greatest achievement in the IOT felid  </p>   </section>   <SearchInput/>   <div className=" parent-post d-flex flex-lg-row-reverse flex-wrap   h-auto justify-content-around     "><div className="post  w-auto" >  <img src={heroBlog} alt=""   /></div><div className="content-post" style={{width:'600px'}}>  <h1>Invest in the intelligence of your home, and enjoy an unparalleledlife experience.  </h1>  <p>  Elevate your living space with cutting-edge IoT technology, transforming your home into a smart   sanctuary tailored to your lifestyle.  Unlock a new realm of possibilities,   where innovation meets tranquility, enriching your everyday experiences like never before  </p>
    </div>
  </div>
  <div className="parent-second  d-flex justify-content-around w-100 flex-wrap " style={{width:'fit-content',height:'fit-content'}}> 
   {   itemData.map((item,i)=>{ return(   <div> <HeroPost item={item}  i={i} /> </div>)   }) }
   </div>{itemData.map((item,i)=>{ return( <PostPage item={item}  i={i} />)})} 
